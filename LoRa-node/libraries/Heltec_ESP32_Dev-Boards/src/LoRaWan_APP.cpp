...
#ifdef REGION_US915
RTC_DATA_ATTR int8_t defaultDrForNoAdr = 3;
#else
...
void LoRaWanClass::join()
{
	if( overTheAirActivation )
	{
		...
		mlmeReq.Req.Join.AppEui = appEui;
		mlmeReq.Req.Join.AppKey = appKey;
		mlmeReq.Req.Join.NbTrials = 5;

		if( LoRaMacMlmeRequest( &mlmeReq ) == LORAMAC_STATUS_OK )
		{
			Serial.println("Join request sent successfully. Waiting for response...");
			deviceState = DEVICE_STATE_SLEEP;
		}
		else
		{
			Serial.println("Join request failed. Retrying...");
			deviceState = DEVICE_STATE_CYCLE;
		}
	}
	else
